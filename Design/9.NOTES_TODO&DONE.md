...
## TODO
- T005) MyObj struct -> MyObj struct&Methods +  DbMyObjIfc + DbMyObj db-struct&db-methods
  ...
  - MyObj        - focuses on thehigher logic of MyObj, that is expected for other packages to use
                 ...
                 - Public methods should internally use DbMyObjIfc, to access r/w data (and never access DbMyObj). 
  ...
  func (c *Collection) CollectionID() string {...use dbCollectionIfc.GetCollectionID()...}
  func (c *Collection) CollectionName() string {...use dbCollectionIfc.GetCollectionName()...}
  ...
  ...
  func (d *DbCollection) GetCollectionID() string {
    d.reload()
    return d.CollectionID
  }
  func (d *DbCollection) GetCollectionName() string {
    d.reload()
