...
func CollectionNew(name string) (*Collection, error) {
	...
	initialColSel, err := colSelectionCreateInitial()
	if err != nil {
		return nil, err
	}
	o := &Collection{}
...
// AppendAndRunColSelection appends ColSelection
func (c *Collection) AppendAndRunColSelection(schema *Schema, jsonInput string, jsonOutput string, requestingUser string) error {
	...
	csel.RegisterObserverCallback(
		func(oldState string, oldError error, xstate *xstate.XState) error {
			c.recalculateStateAndError(csel)
			return nil
		})
	err = csel.run()
	if err != nil {
...
// recalculateStateAndError method:
//
...
func (c *Collection) recalculateStateAndError(csel *ColSelection) {
